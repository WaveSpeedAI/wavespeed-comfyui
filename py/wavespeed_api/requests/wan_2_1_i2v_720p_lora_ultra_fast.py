...
# link api json(wan-2.1-i2v-720p-lora-ultra-fast.json)


class Wan2x1I2V720pLoraUltraFast(BaseRequest):
    """
    ...
    duration: Optional[int] = Field(5, ge=5, le=10, description="Generate video duration length seconds.")  # JSON has step: 5, not directly mappable to Pydantic Field in this way
    guidance_scale: Optional[float] = Field(5.0, ge=1.1, le=10.0, description="The guidance scale for generation.")  # JSON has step: 0.1
    flow_shift: Optional[float] = Field(5.0, ge=1, le=10, description="The shift value for the timestep schedule for flow matching.")  # JSON has step: 0.1
    ...
    def get_api_path(self):
        ...
        return "/api/v2/wavespeed-ai/wan-2.1/i2v-720p-lora-ultra-fast"
